...
TO DO:
1) Calculate Expiry data based on TTL NOTE: Maybe do not need usedByDate since proposal expired based on TTL
2) Check expiry
*/
...
contract Voting {
    ...
    function castVoteForProposal(uint8 vote,uint256 userId ,uint256 pId)public{
      ...
      require(voters[userId].shares>= vote,"user casted votes more than what he owns!");
      if(!voters[userId].voted){
        voters[userId].voted = true;
        if(vote > 0){
            proposals[pId].yesVotes+=vote;
        }else{
            proposals[pId].noVotes++;
